...
impl Character {
    ...
    pub fn vague(&self, date: &Date) -> String {
        ...
        let source = format!(
            ...
            All actions are formatted in present continuous tense. Examples:[Working on personal project. Going for a run. Running around screaming like a schizophrenic racist.]
            ",
            common = self,
            name = self.name(),
            curr_date = date,
    ...
    pub fn decompose(&self, datetime: &DateTime) -> Result<(String, i64, Time), Box<dyn Error>> {
        ...
        if let Some(curr_acction) = surrounding.get(1) {
            ...
            let source = format!(
                ...
                With durations in increments of 5 minutes, list the subtasks (in present continous form) that {name} does when {name} is {action} starting at {start_time}. (Total duration in minutes: {duration}).
