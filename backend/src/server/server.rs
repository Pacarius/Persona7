...
use crate::{
    main, misc::{ollama::ollama::Ollama, sqlite::helper::Helper}, world::{navigation::Navigator, world::World}, CONFIG
};
...
impl Server {
    pub async fn main(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        tokio::spawn(async move {
            Server::accept_clients(to_client_rx.clone(), from_client_tx.clone(), clients, world, &endpoint)
                .await
                .unwrap_or_else(|e| {
        ...
        loop {
            ...
            while let Ok(command) = self.from_client.0.try_recv() {
                ...
                match &command.trim() {
                    ...
                    val if val == &"NEED" => {
                        let message = // format!("{:?}", world.get_map().get_characters().iter().map(|c| json!({"name": c.name(),"position": c.position().to_string(),"plan": format!("{:?}", c.short_term_mem().plan_vague), "sprite": c.sprite()})).collect::<Vec<_>>()));
                        json!({
                            "type": "init",
                            ...
                            "timestamp": world.datetime.to_string()
                        }).to_string();
                        self.sqlite_helper.add_entry(&message);
                        println!("Sending: {}", message);
                        self.to_client.1.send(message);
                    }
                    _ => {}
                }
            }
            ...
            if let Some(output) = relay {
                self.sqlite_helper.add_entry(&output);
                if let Ok(_) = self.to_client.1.send(output.clone()) {
                    // println!("Sent {} through channel.", output);
                }
            };
            sleep(Duration::from_millis(CONFIG.tick_cooldown_ms)).await;
        }
        // }
        Ok(())
    }
