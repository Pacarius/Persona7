...
use std::{collections::HashMap, fmt::Display, fs::{self, exists, read_dir, File}, path};
use indexmap::IndexMap;
use serde::Deserialize;
use serde_json::value::Index;

use crate::ollama::{ollama::Ollama, options::{ChatOptions, ChatRole, Message}};
...
impl Adapter{
    ...
    pub async fn initialise(&self) -> Vec<String>{
        ...
        println!("{}", characters.iter().enumerate().map(|(i, item)|{
            format!("{}: {}\n", i, item.0)
        }).collect::<String>());
        let target = characters.iter()
        .nth({text_io::read!("{}")}).unwrap(); 
    ...
    pub async fn main(&self){
        ...
        loop{
            ...
            let response = self.client.chat(ChatOptions::new(model.to_string(), vec![Message::new(ChatRole::USER.to_string(), chat_log.join("\n"))])).await;
            let response = response["message"]["content"].as_str().unwrap().replace("\\n", "\n");
            println!("{}", response);
