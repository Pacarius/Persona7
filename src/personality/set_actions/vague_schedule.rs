    ...
    pub async fn daily_schedule(&mut self, llama: &Ollama, date: &Date) {
        ...
        if let Some(response_str) = llama.generate(options).await["response"].as_str() {
            if let Ok(response_json) = serde_json::from_str::<Value>(response_str) {
                ...
                if let Some(wake_action) = short_term_mem_mut.plan_vague.0.first().cloned() {
                    if let Some(sleep_action) = short_term_mem_mut.plan_vague.0.last().cloned() {
                        short_term_mem_mut.plan_vague.0.clear();
                        short_term_mem_mut.plan_vague.0.push(wake_action);
                        short_term_mem_mut.plan_vague.0.extend(plans);
                        short_term_mem_mut.plan_vague.0.push(sleep_action);
                    }
                }
            }
        }
