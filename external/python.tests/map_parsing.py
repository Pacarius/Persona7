...
def parse_debug_string(debug_string):
    ...
    print(f"Parsed regions: {regions}")  # Debugging output
    return regions

def generate_image(regions, tile_size=16, output_file="world_debug.png"):
    """
    Generate a 2D image based on the parsed regions and rooms.
    """
    # Determine the overall size of the image
    max_width = max(region["position"][0] + region["size"][0] for region in regions)
    max_height = max(region["position"][1] + region["size"][1] for region in regions)
